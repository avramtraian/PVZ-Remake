...
internal void
Game_InitializeGardenGrid(game_state* GameState)
{
...
Game_Initialize(platform_game_memory* GameMemory)
{
    ...
    //
    // NOTE(Traian): Initialize the game garden grid.
    //

    Game_InitializeGardenGrid(GameState);
...
internal inline f32
Game_GetGardenGridCellPositionX(const game_garden_grid* GardenGrid, u32 CellIndexX)
{
...
internal inline f32
Game_GetGardenGridCellPositionY(const game_garden_grid* GardenGrid, u32 CellIndexY)
{
...
internal inline vec2
Game_GetGardenGridCellPosition(const game_garden_grid* GardenGrid, u32 CellIndexX, u32 CellIndexY)
{
    vec2 Result;
    Result.X = Game_GetGardenGridCellPositionX(GardenGrid, CellIndexX);
    Result.Y = Game_GetGardenGridCellPositionY(GardenGrid, CellIndexY);
    return Result;
}

internal inline s32
Game_GetGardenGridCellIndexX(const game_garden_grid* GardenGrid, f32 PositionX)
{
...
internal inline s32
Game_GetGardenGridCellIndexY(const game_garden_grid* GardenGrid, f32 PositionY)
{
...
Game_PushProjectileEntity(game_garden_grid* GardenGrid, projectile_type Type, u32 CellIndexY)
{
...
Game_PushZombieEntity(game_garden_grid* GardenGrid, zombie_type Type, u32 CellIndexY)
{
...
internal u32
Game_RemovePendingDestroyEntities(void* EntityBuffer, memory_size EntityByteCount, u32 EntityCount)
{
...
internal void
Game_UpdateGardenGrid(game_state* GameState, game_platform_state* PlatformState, f32 DeltaTime)
{
    game_garden_grid* GardenGrid = &GameState->GardenGrid;
    ...
    const vec2 GARDEN_MAX_POINT_PERCENTAGE = Vec2(0.99F, 0.85F);
    ...
    const s32 MouseCellIndexX = Game_GetGardenGridCellIndexX(GardenGrid, GameMousePosition.X);
    const s32 MouseCellIndexY = Game_GetGardenGridCellIndexY(GardenGrid, GameMousePosition.Y);
    ...
    if (GardenGrid->SpawnNextZombieTimer >= GardenGrid->SpawnNextZombieDelay)
    {
        ...
        if (GardenGrid->CellCountY > 0)
        {
            ...
            zombie_entity* ZombieEntity = Game_PushZombieEntity(GardenGrid, ZOMBIE_TYPE_NORMAL, SpawnCellIndexY);
            ZombieEntity->Health = ZOMBIE_NORMAL_HEALTH;
            ZombieEntity->Position.X = GameState->Camera.UnitCountX + 0.2F;
            ZombieEntity->Position.Y = Game_GetGardenGridCellPositionY(GardenGrid, SpawnCellIndexY);
                ...
                switch (PlantEntity->Type)
                {
                    case PLANT_TYPE_SUNFLOWER:
                    {
                        ...
                        if (Sunflower->GenerateTimer >= Sunflower->GenerateDelayBase + DelayOffset)
                        {
                            ...
                            projectile_entity* SunProjectile = Game_PushProjectileEntity(GardenGrid, PROJECTILE_TYPE_SUN,
                                                                                         CellIndexY);
                    ...
                    case PLANT_TYPE_PEASHOOTER:
                    {
                        ...
                        if (Peashooter->ShootTimer >= Peashooter->ShootDelay)
                        {
                            ...
                            if (AreZombiesOnTheLane)
                            {
                                ...
                                projectile_entity* PeaProjectile = Game_PushProjectileEntity(GardenGrid, PROJECTILE_TYPE_PEA,
                                                                                             CellIndexY);
    ...
    for (u32 ZombieIndex = 0; ZombieIndex < GardenGrid->CurrentZombieCount; ++ZombieIndex)
    {
        ...
        const s32 AttackCellIndexX = Game_GetGardenGridCellIndexX(GardenGrid, ZombieFrontPositionX);
    ...
    //
    // NOTE(Traian): Update projectile entities.
    //
        ...
        switch(ProjectileEntity->Type)
        {
            ...
            case PROJECTILE_TYPE_PEA:
            {
                ...
                else
                {
                    ...
                    {
                        zombie_entity* ZombieEntity = GardenGrid->ZombieEntities + ZombieIndex;
                        if (!ZombieEntity->IsPendingDestroy && ZombieEntity->CellIndexY == ProjectileEntity->CellIndexY)
                        {
    ...
    GardenGrid->CurrentProjectileCount = Game_RemovePendingDestroyEntities(GardenGrid->ProjectileEntities,
                                                                           sizeof(projectile_entity),
                                                                           GardenGrid->CurrentProjectileCount);

    GardenGrid->CurrentZombieCount = Game_RemovePendingDestroyEntities(GardenGrid->ZombieEntities,
                                                                       sizeof(zombie_entity),
                                                                       GardenGrid->CurrentZombieCount);
}

internal void
Game_RenderGardenGrid(game_state* GameState, game_platform_state* PlatformState)
{
    game_garden_grid* GardenGrid = &GameState->GardenGrid;
    const f32 InvCellCountX = 1.0F / (f32)GardenGrid->CellCountX;
    const f32 InvCellCountY = 1.0F / (f32)GardenGrid->CellCountY;

    const f32 GRASS_TILE_GRID_Z_OFFSET  = 1.0F;
    const f32 PLANTS_BASE_Z_OFFSET      = 2.0F;
    const f32 ZOMBIES_BASE_Z_OFFSET     = 10.0F;
    const f32 PROJECTILES_BASE_Z_OFFSET = 20.0F;
    ...
    for (u32 CellIndexY = 0; CellIndexY < GardenGrid->CellCountY; ++CellIndexY)
    {
        for (u32 CellIndexX = 0; CellIndexX < GardenGrid->CellCountX; ++CellIndexX)
        {
            ...
            Renderer_PushPrimitive(&GameState->Renderer,
                                   ...
                                   GRASS_TILE_GRID_Z_OFFSET, GRASS_CELL_COLOR_TABLE[CellColorIndex]);
        }
    }

    //
    // NOTE(Traian): Render plant entities.
    //
    ...
    for (u32 CellIndexY = 0; CellIndexY < GardenGrid->CellCountY; ++CellIndexY)
    {
        for (u32 CellIndexX = 0; CellIndexX < GardenGrid->CellCountX; ++CellIndexX)
        {
            ...
            const f32 RenderZOffset = PLANTS_BASE_Z_OFFSET + (GardenGrid->CellCountY - CellIndexY - 1);
    ...
    //
    // NOTE(Traian): Render zombie entities.
    //
    ...
    for (u32 ZombieIndex = 0; ZombieIndex < GardenGrid->CurrentZombieCount; ++ZombieIndex)
    {
        ...
        const f32 RenderZOffset = ZOMBIES_BASE_Z_OFFSET + (GardenGrid->CellCountY - ZombieEntity->CellIndexY - 1);
    ...
    //
    // NOTE(Traian): Render projectile entities.
    //
...
Game_UpdateAndRender(game_state* GameState, game_platform_state* PlatformState, f32 DeltaTime)
{
    ...
    Game_UpdateGardenGrid(GameState, PlatformState, DeltaTime);
    Game_RenderGardenGrid(GameState, PlatformState);
